...
import org.eclipse.jdt.internal.core.search.matching.StringPattern;
...
/**
 * A search pattern defines how search results are found. Use <code>SearchPattern.createPattern</code> to create a
 * search pattern.
 * <p>
 * Search patterns are used during the search phase to decode index entries that were added during the indexing phase
 * (see {@link SearchDocument#addIndexEntry(char[], char[])}). When an index is queried, the index categories and keys
 * to consider are retrieved from the search pattern using {@link #getIndexCategories()} and {@link #getIndexKey()}, as
 * well as the match rule (see {@link #getMatchRule()}). A blank pattern is then created (see
 * {@link #getBlankPattern()}). This blank pattern is used as a record as follows. For each index entry in the given
 * index categories and that starts with the given key, the blank pattern is fed using {@link #decodeIndexKey(char[])}.
 * The original pattern is then asked if it matches the decoded key using {@link #matchesDecodedKey(SearchPattern)}. If
 * it matches, a search document is created for this index entry using {@link SearchParticipant#getDocument(String)}.
 *
 * </p>
 * <p>
 * This class is intended to be sub-classed by clients. A default behavior is provided for each of the methods above,
 * that clients can override if they wish.
 * </p>
 *
...
public abstract class SearchPattern {
	...
	/**
	 * Match rule: The search pattern matches exactly the search result, that is, the source of the search result equals
	 * the search pattern.
	 */
	...
	/**
	 * Match rule: The search pattern contains one or more wild cards ('*' or '?'). A '*' wild-card can replace 0 or
	 * more characters in the search result. A '?' wild-card replaces exactly 1 character in the search result.
	 */
	 ...
	 * <p>
	 * <b>Warning:</b> Implemented only for module declaration search. The support for this rule is <b>not yet
	 * implemented for others</b>
	 * </p>
	 */
	public static final int R_REGEXP_MATCH = 0x0004;

	/**
	 * Match rule: The search pattern matches the search result only if cases are the same. Can be combined to previous
	 * rules, e.g. {@link #R_EXACT_MATCH} | {@link #R_CASE_SENSITIVE}
	 */
	public static final int R_CASE_SENSITIVE = 0x0008;

	/**
	 * Match rule: The search pattern matches search results as raw/parameterized types/methods with same erasure. This
	 * mode has no effect on other java elements search.<br>
	 * Type search example:
	 * <ul>
	 ...
	 * </ul>
	 * Can be combined to all other match rules, e.g. {@link #R_CASE_SENSITIVE} | {@link #R_ERASURE_MATCH} This rule is
	 * not activated by default, so raw types or parameterized types with same erasure will not be found for pattern
	 * List&lt;String&gt;, Note that with this pattern, the match selection will be only on the erasure even for
	 * parameterized types.
	 *
	 * @since 3.1
	 */
	 ...
	 * parameters. This mode has no effect on other java elements search.<br>
	 * Type search example:
	 * <ul>
	 * <li>pattern: <code>List&lt;Exception&gt;</code></li>
	 * <li>match:
	 ...
	 * </ul>
	 * </ul>
	 * Can be combined to all other match rules, e.g. {@link #R_CASE_SENSITIVE} | {@link #R_EQUIVALENT_MATCH} This rule
	 * is not activated by default, so raw types or equivalent parameterized types will not be found for pattern
	 * List&lt;String&gt;, This mode is overridden by {@link #R_ERASURE_MATCH} as erasure matches obviously include
	 * equivalent ones. That means that pattern with rule set to {@link #R_EQUIVALENT_MATCH} | {@link #R_ERASURE_MATCH}
	 ...
	 * </p>
	 * <ul>
	 * <li>'NPE' type string pattern will match 'NullPointerException' and 'NoPermissionException' types,</li>
	 * <li>'NuPoEx' type string pattern will only match 'NullPointerException' type.</li>
	 * </ul>
	 *
	 * This rule is not intended to be combined with any other match rule. In case of other match rule flags are
	 * combined with this one, then match rule validation will return a modified rule in order to perform a better
	 * appropriate search request (see {@link #validateMatchRule(String, int)} for more details).
	 *
	...
	/**
	 * Match rule: The search pattern contains a Camel Case expression with a strict expected number of parts. <br>
	 * Examples:
	 * <ul>
	 ...
	 * Examples:
	 * <ul>
	 * <li>'bar' string pattern will match 'bar1', 'Bar' and 'removeBar' types,</li>
	 * </ul>
	 *
	 * This rule is not intended to be combined with any other match rule. In case of other match rule flags are
	 * combined with this one, then match rule validation will return a modified rule in order to perform a better
	 ...
	 * Examples:
	 * <ul>
	 * <li>'addlist' string pattern will match 'addListener' and 'addChangeListener'</li>
	 * </ul>
	 *
	 * This rule is not intended to be combined with any other match rule. In case of other match rule flags are
	 * combined with this one, then match rule validation will return a modified rule in order to perform a better
	 ...
	 * qualifier. This is optional. In absence all indexes provided by scope will be searched. <br>
	 ...
	 *
	 * @noreference This field is not intended to be referenced by clients.
	 * @see QualifierQuery#encodeQuery(org.eclipse.jdt.internal.core.search.indexing.QualifierQuery.QueryCategory[],
	 *      char[], char[])
	 */
	...
	/**
	 * Creates a search pattern with the rule to apply for matching index keys. It can be exact match, prefix match,
	 * pattern match or regexp match. Rule can also be combined with a case sensitivity flag.
	 *
	 * @param matchRule
	 ...
	 *            For example,
	 *            <ul>
	 *            <li>{@link #R_EXACT_MATCH} | {@link #R_CASE_SENSITIVE}: if an exact and case sensitive match is
	 *            requested,</li>
	 *            <li>{@link #R_PREFIX_MATCH} if a case insensitive prefix match is requested</li>
	 *            <li>{@link #R_EXACT_MATCH} | {@link #R_ERASURE_MATCH}: if a case insensitive and erasure match is
	 *            requested.</li>
	 *            </ul>
	 *            Note that {@link #R_ERASURE_MATCH} or {@link #R_EQUIVALENT_MATCH} has no effect on non-generic
	 *            types/methods search.
	 *            <p>
	 ...
	 * <p>
	 * CamelCase denotes the convention of writing compound names without spaces, and capitalizing every term. This
	 * function recognizes both upper and lower CamelCase, depending whether the leading character is capitalized or
	 * not. The leading part of an upper CamelCase pattern is assumed to contain a sequence of capitals which are
	 * appearing in the matching name; e.g. 'NPE' will match 'NullPointerException', but not 'NewPerfData'. A lower
	 * CamelCase pattern uses a lowercase first character. In Java, type names follow the upper CamelCase convention,
	 * whereas method or field names follow the lower CamelCase convention.
	 * <p>
	 * The pattern may contain lowercase characters, which will be matched in a case sensitive way. These characters
	 * must appear in sequence in the name. For instance, 'NPExcep' will match 'NullPointerException', but not
	 * 'NullPointerExCEPTION' or 'NuPoEx' will match 'NullPointerException', but not 'NoPointerException'.
	 * <p>
	 * Digit characters are treated in a special way. They can be used in the pattern but are not always considered as
	 * leading character. For instance, both 'UTF16DSS' and 'UTFDSS' patterns will match 'UTF16DocumentScannerSupport'.
	 * <p>
	 ...
	 * Examples:
	 * <ol>
	 * <li>pattern = "NPE" name = NullPointerException / NoPermissionException result => true</li>
	 * <li>pattern = "NuPoEx" name = NullPointerException result => true</li>
	 * <li>pattern = "npe" name = NullPointerException result => false</li>
	 * <li>pattern = "IPL3" name = "IPerspectiveListener3" result => true</li>
	 * <li>pattern = "HM" name = "HashMapEntry" result => true</li>
	 * <li>pattern = "HMap" name = "HatMapper" result => true</li>
	 * </ol>
	 *
	 ...
	 * leading character. For instance, both 'UTF16DSS' and 'UTFDSS' patterns will match 'UTF16DocumentScannerSupport'.
	 * <p>
	 * CamelCase can be restricted to match only the same count of parts. When this restriction is specified the given
	 * pattern and the given name must have <b>exactly</b> the same number of parts (i.e. the same number of uppercase
	 * characters).<br>
	 * For instance, 'HM' , 'HaMa' and 'HMap' patterns will match 'HashMap' and 'HatMapper' <b>but not</b>
	 * 'HashMapEntry'.
	 * <p>
	 * Examples:
	 * <ol>
	 ...
	 * <li>pattern = "npe" name = NullPointerException result => false</li>
	 * <li>pattern = "IPL3" name = "IPerspectiveListener3" result => true</li>
	 * <li>pattern = "HM" name = "HashMapEntry" result => (samePartCount == false)</li>
	 * </ol>
	 *
	 * @see #camelCaseMatch(String, int, int, String, int, int, boolean) for algorithm implementation
	 *
	 ...
	 * char[] CamelCase matching does NOT accept explicit wild-cards '*' and '?' and is inherently case sensitive. Can
	 * match only subset of name/pattern, considering end positions as non-inclusive. The sub-pattern is defined by the
	 * patternStart and patternEnd positions.
	 * <p>
	 * CamelCase denotes the convention of writing compound names without spaces, and capitalizing every term. This
	 * function recognizes both upper and lower CamelCase, depending whether the leading character is capitalized or
	 * not. The leading part of an upper CamelCase pattern is assumed to contain a sequence of capitals which are
	 ...
	 * For instance, 'HM' , 'HaMa' and 'HMap' patterns will match 'HashMap', 'HatMapper' <b>and also</b> 'HashMapEntry'.
	 * <ol>
	 * <li>pattern = "NPE" patternStart = 0 patternEnd = 3 name = NullPointerException nameStart = 0 nameEnd = 20 result
	 * => true</li>
	 * <li>pattern = "NPE" patternStart = 0 patternEnd = 3 name = NoPermissionException nameStart = 0 nameEnd = 21
	 * result => true</li>
	 * <li>pattern = "NuPoEx" patternStart = 0 patternEnd = 6 name = NullPointerException nameStart = 0 nameEnd = 20
	 * result => true</li>
	 * <li>pattern = "NuPoEx" patternStart = 0 patternEnd = 6 name = NoPermissionException nameStart = 0 nameEnd = 21
	 * result => false</li>
	 * <li>pattern = "npe" patternStart = 0 patternEnd = 3 name = NullPointerException nameStart = 0 nameEnd = 20 result
	 * => false</li>
	 * <li>pattern = "IPL3" patternStart = 0 patternEnd = 3 name = "IPerspectiveListener3" nameStart = 0 nameEnd = 21
	 * result => true</li>
	 * <li>pattern = "HM" patternStart = 0 patternEnd = 2 name = "HashMapEntry" nameStart = 0 nameEnd = 12 result =>
	 * true</li>
	 * <li>pattern = "HMap" patternStart = 0 patternEnd = 4 name = "HatMapper" nameStart = 0 nameEnd = 9 result =>
	 * true</li>
	 * </ol>
	 *
	 * @param pattern
	 ...
	 * result => true</li>
	 * <li>pattern = "HM" patternStart = 0 patternEnd = 2 name = "HashMapEntry" nameStart = 0 nameEnd = 12 result =>
	 * (samePartCount == false)</li>
	 * </ol>
	 *
	 * @see CharOperation#camelCaseMatch(char[], int, int, char[], int, int, boolean) from which algorithm
	 *      implementation has been entirely copied.
	 *
	 * @param pattern
	 *            the given pattern
	 * @param patternStart
	 ...
	 * Examples:
	 * <ol>
	 * <li>pattern = "NPE" name = NullPointerException / NoPermissionException matchRule = {@link #R_CAMELCASE_MATCH}
	 * result: { 0, 1, 4, 1, 11, 1 } / { 0, 1, 2, 1, 12, 1 }</li>
	 * <li>pattern = "NuPoEx" name = NullPointerException matchRule = {@link #R_CAMELCASE_MATCH} result: { 0, 2, 4, 2,
	 * 11, 2 }</li>
	 * <li>pattern = "IPL3" name = "IPerspectiveListener3" matchRule = {@link #R_CAMELCASE_MATCH} result: { 0, 2, 12, 1,
	 * 20, 1 }</li>
	 * <li>pattern = "HashME" name = "HashMapEntry" matchRule = {@link #R_CAMELCASE_MATCH} result: { 0, 5, 7, 1 }</li>
	 * <li>pattern = "N???Po*Ex?eption" name = NullPointerException matchRule = {@link #R_PATTERN_MATCH} |
	 * {@link #R_CASE_SENSITIVE} result: { 0, 1, 4, 2, 11, 2, 14, 6 }</li>
	 * <li>pattern = "Ha*M*ent*" name = "HashMapEntry" matchRule = {@link #R_PATTERN_MATCH} result: { 0, 2, 4, 1, 7, 3
	 * }</li>
	 * </ol>
	 *
	 * @see #camelCaseMatch(String, String, boolean) for more details on the camel case behavior
	 * @see CharOperation#match(char[], char[], boolean) for more details on the pattern match behavior
	 *
	 * @param pattern
	 ...
	 *            Some examples:
	 *            <ul>
	 *            <li>{@link #R_EXACT_MATCH} | {@link #R_CASE_SENSITIVE}: if an exact case sensitive match is
	 *            expected,</li>
	...
	/**
	 * Returns a search pattern that combines the given two patterns into an "and" pattern. The search result will match
	 * both the left pattern and the right pattern.
	 *
	 * @param leftPattern
	 *            the left pattern
	 * @param rightPattern
	 ...
	 *             "https://bugs.eclipse.org/bugs/show_bug.cgi?id=142044" for more details). This might be done in a
	 *             further version...
	 */
	...
	private static SearchPattern createFieldPattern(String patternString, int limitTo, int matchRule) {
					...
					switch (token) {
						...
						case TerminalTokens.TokenNameWHITESPACE:
							if (!(TerminalTokens.TokenNameWHITESPACE == lastToken
								...
								mode = InsideType;
							break;
						default: // all other tokens are considered identifiers (see bug 21763 Problem in Java search
									// [search])
							if (fieldName == null)
		...
		return new FieldPattern(fieldNameChars, declaringTypeQualification, declaringTypeSimpleName, typeQualification,
				typeSimpleName, limitTo, matchRule);
	}

	private static SearchPattern createStringPattern(String patternString, int limitTo, int matchRule) {
		Scanner scanner = new Scanner(false /* comment */, true /* whitespace */, false /* nls */,
				ClassFileConstants.JDK1_7/* sourceLevel */, null /* taskTags */, null/* taskPriorities */,
				true/* taskCaseSensitive */);
		...
		while (token != TerminalTokens.TokenNameEOF) {
			switch (token) {
				case TerminalTokens.TokenNameIdentifier:
					// Extracting the string literal
					if (stringContent == null) {
						stringContent = scanner.getCurrentTokenString();
					} else {
						stringContent += scanner.getCurrentTokenString();
					}
					break;
		...
		if (stringContent == null)
			return null;
		// Create a custom StringPattern (or similar) here
		// For now, using FieldPattern for demonstration
		// You might want to create a new pattern class similar to FieldPattern but for strings.
		return new StringPattern(stringContent, limitTo,  matchRule);

	}

//	private static SearchPattern createStringPattern(String patternString, int limitTo, int matchRule) {
//		// Use similar tokenization and parsing logic as in the createFieldPattern method
//		Scanner scanner = new Scanner(false /* comment */, true /* whitespace */, false /* nls */,
//				ClassFileConstants.JDK1_7/* sourceLevel */, null /* taskTags */, null/* taskPriorities */,
//				true/* taskCaseSensitive */);
//		scanner.setSource(patternString.toCharArray());
...
//					if (stringName == null) {
//						stringName = scanner.getCurrentTokenString();
//					} else {
//						stringName += scanner.getCurrentTokenString();
//					}
//					break;
//				// Handle other token types as needed
//				// For example, whitespace, dots, etc.
//			}
//
//			try {
//				token = scanner.getNextToken();
...
//		}
//
//		char[] stringNameChars = stringName.toCharArray();
//
//		// Create the StringPattern object
//		return new StringPattern(stringNameChars, matchRule);
//	}
	...
	private static SearchPattern createMethodOrConstructorPattern(String patternString, int limitTo, int matchRule,
					...
					if (argCount == 0) {
						switch (token) {
							...
							case TerminalTokens.TokenNameWHITESPACE:
								...
								break;
							default: // all other tokens are considered identifiers (see bug 21763 Problem in Java
										// search [search])
								if (selector == null)
					...
					switch (token) {
						...
						case TerminalTokens.TokenNameUNSIGNED_RIGHT_SHIFT:
							...
							if (argCount == 0) {
								...
								typeArguments = Signature.getTypeArguments(
										Signature.createTypeSignature(pseudoType, false).toCharArray());
								mode = InsideSelector;
							}
							break;
						case TerminalTokens.TokenNameLESS:
								...
								if (parameterTypes != null) {
									if (parameterTypes.length == parameterCount)
										System.arraycopy(parameterTypes, 0,
												parameterTypes = new String[parameterCount * 2], 0, parameterCount);
				...
				try {
					if (parameterTypes != null) {
						...
						if (parameterTypeSignatures[i].indexOf(Signature.C_GENERIC_START) < 0) {
							...
							parameterTypePart = Signature
									.toCharArray(Signature.getTypeErasure(parameterTypeSignatures[i].toCharArray()));
						}
					}
				} catch (IllegalArgumentException iae) {
		...
		if (isConstructor) {
			return new ConstructorPattern(declaringTypeSimpleName, declaringTypeQualification, declaringTypeSignature,
	...
	/**
	 * Returns a search pattern that combines the given two patterns into an "or" pattern. The search result will match
	 * either the left pattern or the right pattern.
	 *
	 * @param leftPattern
	 *            the left pattern
	 * @param rightPattern
	...
	/**
	 * Returns a search pattern based on a given string pattern. The string patterns support '*' wild-cards. The
	 ...
	 *            </ul>
	 *            <p>
	 *            Type arguments can be specified to search for references to parameterized types using following
	 *            syntax:
	 *            </p>
	 *            <p>
	 *            <b><code>'&lt;' { [ '?' {'extends'|'super'} ] type ( ',' [ '?' {'extends'|'super'} ] type )* | '?' } '&gt;'</code></b>
	 *            </p>
	 *            <div style="font-style:italic;"> Note that:
	 *            <ul>
	 *            <li>'*' is not valid inside type arguments definition &lt;&gt;</li>
	 *            <li>'?' is treated as a wildcard when it is inside &lt;&gt; (i.e. it must be put on first position of
	 *            the type argument)</li>
	 *            </ul>
	 *            </div> Since 3.14 for Java 9, Type Declaration Patterns can have module names also embedded with the
	 *            following syntax
	 *            <p>
	 *            <b><code>[moduleName1[,moduleName2,..]]/[qualification '.']typeName ['&lt;' typeArguments '&gt;']</code></b>
	 *            </p>
	 *            <p>
	 *            Unnamed modules can also be included and are represented either by an absence of module name
	 *            implicitly or explicitly by specifying ALL-UNNAMED for module name. Module graph search is also
	 *            supported with the limitTo option set to <code>IJavaSearchConstants.MODULE_GRAPH</code>. In the module
	 *            graph case, the given type is searched in all the modules required directly as well as indirectly by
	 *            the given module(s).
	 *            </p>
	 *            <p>
	 *            Note that whitespaces are ignored in between module names. It is an error to give multiple module
	 *            separators - in such cases a null pattern will be returned.
	 *            </p>
	 *            <p>
	 *            Examples:
	 *            </p>
	 ...
	 *            <p>
	 *            <b><code>[declaringType '.'] ['&lt;' typeArguments '&gt;'] methodName ['(' parameterTypes ')'] [returnType]</code></b>
	 *            </p>
	 *            <p>
	 ...
	 *            <p>
	 *            <b><code>['&lt;' typeArguments '&gt;'] [declaringQualification '.'] typeName ['(' parameterTypes ')']</code></b>
	 *            </p>
	 *            <p>
	 *            Type arguments have the same syntax as explained in the type patterns section.
	 *            </p>
	 *            <p>
	 *            <i>Note that the constructor name should not be entered as it is always the same as the type name.</i>
	 *            </p>
	 *            <p>
	 *            Examples:
	 *            </p>
	 ...
	 *            <li>{@link IJavaSearchConstants#TYPE}: look for all types (i.e. classes, interfaces, enum and
	 *            annotation types)</li>
	 ...
	 *            <ul>
	 *            <li>{@link IJavaSearchConstants#DECLARATIONS DECLARATIONS}: will search declarations matching with the
	 *            corresponding element. In case the element is a method, declarations of matching methods in sub-types
	 *            will also be found, allowing to find declarations of abstract methods, etc.<br>
	 *            Note that additional flags {@link IJavaSearchConstants#IGNORE_DECLARING_TYPE IGNORE_DECLARING_TYPE}
	 *            and {@link IJavaSearchConstants#IGNORE_RETURN_TYPE IGNORE_RETURN_TYPE} are ignored for string
	 *            patterns. This is due to the fact that client may omit to define them in string pattern to have same
	 *            behavior.</li>
	 *            <li>{@link IJavaSearchConstants#REFERENCES REFERENCES}: will search references to the given
	 *            element.</li>
	 *            <li>{@link IJavaSearchConstants#ALL_OCCURRENCES ALL_OCCURRENCES}: will search for either declarations
	 *            or references as specified above.</li>
	 *            <li>{@link IJavaSearchConstants#IMPLEMENTORS IMPLEMENTORS}: for types, will find all types which
	 *            directly implement/extend a given interface. Note that types may be only classes or only interfaces if
	 *            {@link IJavaSearchConstants#CLASS CLASS} or {@link IJavaSearchConstants#INTERFACE INTERFACE} is
	 *            respectively used instead of {@link IJavaSearchConstants#TYPE TYPE}.</li>
	 *            <li>{@link IJavaSearchConstants#MODULE_GRAPH MODULE_GRAPH}: for types with a module prefix, will find
	 *            all types present in required modules (directly or indirectly required) ie in any module present in
	 *            the module graph of the given module.</li>
	 ...
	 *            <td>Return only type references used as a type argument in a parameterized type or a parameterized
	 *            method.
	 *            <tr>
	 ...
	 *            <td>Return only super field accesses or super method invocations (e.g. using the <code>super</code>
	 *            qualifier).
	 *            <tr>
	 ...
	 *            <td>Return only primary field accesses or primary method invocations (e.g. using the <code>this</code>
	 *            qualifier).
	 *            <tr>
	 ...
	 *
	 *            <p>
	 *            Note that {@link #R_REGEXP_MATCH} is supported since 3.14 for the special case of
	 *            {@link IJavaSearchConstants#DECLARATIONS DECLARATIONS} search of {@link IJavaSearchConstants#MODULE
	 *            MODULE}
	 *            </p>
	 *            <p>
	...
	public static SearchPattern createPattern(String stringPattern, int searchFor, int limitTo, int matchRule) {
		...
		switch (searchFor) {
			...
			case IJavaSearchConstants.METHOD:
				return createMethodOrConstructorPattern(stringPattern, limitTo, matchRule,
			...
			case IJavaSearchConstants.STRING:
				return createStringPattern(stringPattern, limitTo, matchRule);
		}
		return null;
	}
	 ...
	 * <br>
	 * Note that for generic searches, the returned pattern consider {@link #R_ERASURE_MATCH} matches. If other kind of
	 * generic matches (i.e. {@link #R_EXACT_MATCH} or {@link #R_EQUIVALENT_MATCH}) are expected,
	 * {@link #createPattern(IJavaElement, int, int)} method need to be used instead with the explicit match rule
	 * specified. <br>
	 ...
	 *            <li>{@link IJavaSearchConstants#DECLARATIONS DECLARATIONS}: will search declarations matching with the
	 *            corresponding element. In case the element is a method, declarations of matching methods in sub-types
	 *            will also be found, allowing to find declarations of abstract methods, etc. Some additional flags may
	 *            be specified while searching declaration:
	 *            <ul>
	 *            <li>{@link IJavaSearchConstants#IGNORE_DECLARING_TYPE IGNORE_DECLARING_TYPE}: declaring type will be
	 *            ignored during the search.<br>
	 ...
	 *            </pre>
	 *
	 *            search for <code>method</code> declaration with this flag will return 2 matches: in A and in C</li>
	 *            <li>{@link IJavaSearchConstants#IGNORE_RETURN_TYPE IGNORE_RETURN_TYPE}: return type will be ignored
	 *            during the search.<br>
	 *            Using same example, search for <code>method</code> declaration with this flag will return 2 matches:
	 *            in A and in B.</li>
	 *            </ul>
	 *            Note that these two flags may be combined and both declaring and return types can be ignored during
	 *            the search. Then, using same example, search for <code>method</code> declaration with these 2 flags
	 *            will return 3 matches: in A, in B and in C</li>
	 *            <li>{@link IJavaSearchConstants#REFERENCES REFERENCES}: will search references to the given
	 *            element.</li>
	 *            <li>{@link IJavaSearchConstants#ALL_OCCURRENCES ALL_OCCURRENCES}: will search for either declarations
	 *            or references as specified above.</li>
	...
	public static SearchPattern createPattern(IJavaElement element, int limitTo, int matchRule) {
		...
		switch (element.getElementType()) {
			case IJavaElement.FIELD:
				...
				// If field is record's component, create a OR pattern comprising of record's component and its accessor
				// methods
				IType declaringType = field.getDeclaringType();
				try {
					if (declaringType.isRecord()) {
						MethodPattern accessorMethodPattern = new MethodPattern(name, declaringQualification,
								declaringSimpleName, typeQualification, typeSimpleName, null, null,
								field.getDeclaringType(), limitTo, matchRule);
			...
			case IJavaElement.TYPE:
				...
				searchPattern = createTypePattern(simpleName, type.getPackageFragment().getElementName().toCharArray(),
				...
				if ((maskedLimitTo == IJavaSearchConstants.DECLARATIONS)
					...
					qualifiedName = CharOperation.equals(simpleName, qualifiedName) ? CharOperation.NO_CHAR
							: qualifiedName;
					MatchLocator.setIndexQualifierQuery(searchPattern, QualifierQuery
							.encodeQuery(new QueryCategory[] { QueryCategory.REF }, simpleName, qualifiedName));
				}
				break;
	 ...
	 * {@link #matchesDecodedKey(SearchPattern)} to find out if the corresponding index entry should be considered.
	 * <p>
	 ...
	 * Query a given index for matching entries. Assumes the sender has opened the index and will close when finished.
	 *
	 * This API provides a flag to control whether to skip resolving document name for the matching entries. If a
	 * SearchPattern subclass has a different implementation of index matching, they have to override this API to
	 * support document name resolving feature.
	 *
	 * @param index
	 *            the target index to query
	 * @param requestor
	 *            the search requestor
	 * @param participant
	 *            the search participant
	 * @param scope
	 *            the search scope where the search results should be found
	 * @param resolveDocumentName
	 *            whether to skip the document name resolving for the matching entries
	 * @param monitor
	 *            a progress monitor
	 *
	 * @noreference This method is not intended to be referenced by clients.
	...
	/**
	 * Returns a key to find in relevant index categories, if null then all index entries are matched. The key will be
	 * matched according to some match rule. These potential matches will be further narrowed by the match locator, but
	 * precise match locating can be expensive, and index query should be as accurate as possible so as to eliminate
	 * obvious false hits.
	 * <p>
	...
	/**
	 * Returns an array of index categories to consider for this index query. These potential matches will be further
	 ...
	 * match. Rule can also be combined with a case sensitivity flag.
	 *
	 * @return one of R_EXACT_MATCH, R_PREFIX_MATCH, R_PATTERN_MATCH, R_REGEXP_MATCH combined with R_CASE_SENSITIVE,
	 *         e.g. R_EXACT_MATCH | R_CASE_SENSITIVE if an exact and case sensitive match is requested, or
	 *         R_PREFIX_MATCH if a prefix non case sensitive match is requested.
	 */
	 ...
	 * In certain circumstances described in the table below, the returned match rule is modified in order to provide a
	 * more efficient search pattern:
	 * <ol>
	 * <li>when the {@link #R_REGEXP_MATCH} flag is set, then <b>the pattern is rejected</b> as this kind of match is
	 * not supported yet and <code>-1</code> is returned).</li>
	 * <li>when the string pattern has <u>no</u> pattern characters (e.g. '*' or '?') and the pattern match flag is set
	 * (i.e. the match rule has the {@link #R_PATTERN_MATCH} flag), then <b>the pattern match flag is reset</b>.<br>
	 * Reversely, when the string pattern has pattern characters and the pattern match flag is <u>not</u> set, then
	 * <b>the pattern match flag is set</b>.</li>
	 * <li>when the {@link #R_PATTERN_MATCH} flag is set then, <b>other {@link #R_PREFIX_MATCH},
	 * {@link #R_CAMELCASE_MATCH} or {@link #R_CAMELCASE_SAME_PART_COUNT_MATCH} flags are reset</b> if they are
	 * tentatively combined.</li>
	 * <li>when the {@link #R_CAMELCASE_MATCH} flag is set, then <b>other {@link #R_PREFIX_MATCH} or
	 * {@link #R_CAMELCASE_SAME_PART_COUNT_MATCH} flags are reset</b> if they are tentatively combined.<br>
	 * Reversely, if the string pattern cannot be a camel case pattern (i.e. contains invalid Java identifier characters
	 * or does not have at least two uppercase characters - one for method camel case patterns), then <b>the CamelCase
	 * match flag is replaced with a prefix match flag</b>.</li>
	 * <li>when the {@link #R_CAMELCASE_SAME_PART_COUNT_MATCH} flag is set, then <b>({@link #R_PREFIX_MATCH} flag is
	 * reset</b> if it's tentatively combined.<br>
	 * Reversely, if the string pattern cannot be a camel case pattern (i.e. contains invalid Java identifier characters
	 * or does not have at least two uppercase characters - one for method camel case patterns), then <b>the CamelCase
	 * part count match flag is reset</b>.</li>
	 * </ol>
	 * <i>Note: the rules are validated in the documented order. For example, it means that as soon as the string
	 * pattern contains one pattern character, the pattern match flag will be set and all other match flags reset:
	 * validation of rule 2) followed by rule 3)...</i>
	 *
	 * @param stringPattern
	 *            The string pattern
	 * @param matchRule
